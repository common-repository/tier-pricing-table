	...
	public function __construct() {
		...
		add_filter( 'tiered_pricing_table/price/pricing_rule', function ( PricingRule $pricingRule, $productId ) {
			...
			if ( has_action( 'tier_pricing_table/price/minimum' ) ) {
				wc_deprecated_hook( 'tier_pricing_table/price/minimum', '3.0.0',
					'tiered_pricing_table/price/pricing_rule' );
				$hasDeprecated = true;
			}

			if ( has_action( 'tier_pricing_table/price/type' ) ) {
				wc_deprecated_hook( 'tier_pricing_table/price/type', '3.0.0',
					'tiered_pricing_table/price/pricing_rule' );
				$hasDeprecated = true;
			}

			if ( has_action( 'tier_pricing_table/price/product_price_rules' ) ) {
				wc_deprecated_hook( 'tier_pricing_table/price/product_price_rules', '3.0.0',
					'tiered_pricing_table/price/pricing_rule' );
				$hasDeprecated = true;
			}
			...
			$rules       = apply_filters( 'tier_pricing_table/price/product_price_rules', $rules, $productId,
				$pricingType, $productId );
		...
		add_filter( 'tiered_pricing_table/supported_simple_product_types', function ( $types ) {
			if ( has_action( 'tier_pricing_table/supported_simple_product_types' ) ) {
				wc_deprecated_hook( 'tier_pricing_table/supported_simple_product_types', '3.0.0',
					'tiered_pricing_table/supported_simple_product_types' );
			} else {
				return $types;
			}

			return apply_filters( 'tier_pricing_table/supported_simple_product_types', $types );
		} );
		...
		add_filter( 'tiered_pricing_table/supported_variable_product_types', function ( $types ) {
			if ( has_action( 'tier_pricing_table/supported_variable_product_types' ) ) {
				wc_deprecated_hook( 'tier_pricing_table/supported_variable_product_types', '3.0.0',
					'tiered_pricing_table/supported_variable_product_types' );
			} else {
				return $types;
			}

			return apply_filters( 'tier_pricing_table/supported_variable_product_types', $types );
		} );

		add_filter( 'tiered_pricing_table/price/price_by_rules',
			function ( $productPrice, $quantity, $product_id, $context ) {
				if ( has_action( 'tier_pricing_table/price/price_by_rules' ) ) {
					wc_deprecated_hook( 'tier_pricing_table/price/price_by_rules', '3.0.0',
						'tiered_pricing_table/price/price_by_rules' );
				} else {
					return $productPrice;
				}

				return apply_filters( 'tier_pricing_table/price/price_by_rules', $productPrice, $quantity, $product_id,
					$context );
			}, 999, 5 );
		...
		add_filter( 'tiered_pricing_table/cart/need_price_recalculation', function ( $needRecalculation, $cartItem ) {
			if ( has_action( 'tier_pricing_table/cart/need_price_recalculation' ) ) {
				wc_deprecated_hook( 'tier_pricing_table/cart/need_price_recalculation', '3.0.0',
			 ...
			 * @since 5.0.2
			 */
			return apply_filters( 'tier_pricing_table/cart/need_price_recalculation', $needRecalculation, $cartItem );
		}, 999, 2 );

		add_filter( 'tiered_pricing_table/cart/need_price_recalculation/item',
			function ( $needRecalculation, $cartItem ) {
				if ( has_action( 'tier_pricing_table/cart/need_price_recalculation/item' ) ) {
					wc_deprecated_hook( 'tier_pricing_table/cart/need_price_recalculation/item', '3.0.0',
						'tiered_pricing_table/cart/need_price_recalculation/item' );
				} else {
					return $needRecalculation;
				}

				return apply_filters( 'tier_pricing_table/cart/need_price_recalculation/item', $needRecalculation,
					$cartItem );
			}, 999, 2 );
		...
		add_filter( 'tiered_pricing_table/cart/total_product_count', function ( $count, $cartItem ) {
			if ( has_action( 'tier_pricing_table/cart/total_product_count' ) ) {
				wc_deprecated_hook( 'tier_pricing_table/cart/total_product_count', '3.0.0',
		...
		add_filter( 'tiered_pricing_table/cart/product_cart_price', function ( $new_price, $cartItem, $key ) {
			if ( has_action( 'tier_pricing_table/cart/product_cart_price' ) ) {
				wc_deprecated_hook( 'tier_pricing_table/cart/product_cart_price', '3.0.0',
		...
		add_filter( 'tiered_pricing_table/cart/product_cart_price/item', function ( $new_price, $cartItem ) {
			if ( has_action( 'tier_pricing_table/cart/product_cart_price/item' ) ) {
				wc_deprecated_hook( 'tier_pricing_table/cart/product_cart_price/item', '3.0.0',
					'tiered_pricing_table/cart/product_cart_price/item' );
			} else {
				return $new_price;
			}
